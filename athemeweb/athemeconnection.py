...
class AthemeNickServMethods(object):
	...
	def list_own_groups(self):
		return self._parse_access(self.parent.atheme.command(self.parent.authcookie, self.parent.username, self.parent.ipaddr, 'NickServ', 'LISTGROUPS'))
...
class AthemeGroupServMethods(object):
	"""
	Parse Atheme GroupServ responses.  Since the XML interface provides the same output as the IRC interface, we
	have to do this.  It's kind of a pain in the ass.
	"""
	def __init__(self, parent):
		self.parent = parent
		self.flags = ['PUBLIC', 'OPEN' ]

	def get_access_list(self, group):
		data = self.parent.atheme.command(self.parent.authcookie, self.parent.username, self.parent.ipaddr, 'GroupServ', 'FLAGS', group)
		raw_lines = data.split('\n')
	...
	def get_access_flags(self, group, nick):
		return self.parent.atheme.command(self.parent.authcookie, self.parent.username, self.parent.ipaddr, 'GroupServ', 'FLAGS', group, nick)

	def set_access_flags(self, group, nick, flags):
		self.parent.atheme.command(self.parent.authcookie, self.parent.username, self.parent.ipaddr, 'GroupServ', 'FLAGS', group, nick, '=' + flags)

	def get_group_info(self, group):
		data = self.parent.atheme.command(self.parent.authcookie, self.parent.username, self.parent.ipaddr, 'GroupServ', 'INFO', group)
		raw_lines = data.split('\n')
	...
	def get_group_flags(self, group):
		data = self.get_group_info(group)
		flags = data['Flags']
	...
	def set_group_flag(self, group, flag, value):
		self.parent.atheme.command(self.parent.authcookie, self.parent.username, self.parent.ipaddr, 'GroupServ', 'SET', group, flag, value)

	def register(self, group):
		try:
			return self.atheme.command('', '', '', 'GroupServ', 'REGISTER', group)
		except Fault, e:

...
class GroupRoot(object):
	def list(self):
		...
		return Template('mygroups_list').render(webinfo=webinfo, conn=conn)
	...
	def info(self, group):
		...
		return Template('groupinfo').render(webinfo=webinfo, conn=conn, group=group)

	def edit_flags(self, group, nick=''):
		try:
			conn = get_xmlrpc_connection()
		except:
			webinfo.response.status = "302 Found"
		...
		return Template('groupeditflags').render(webinfo=webinfo, conn=conn, group=group, nick=nick)

	def set_flags(self, group, nick, flags):
		try:
			conn = get_xmlrpc_connection()
		except:
			webinfo.response.status = "302 Found"
		...
		conn.chanserv.set_access_flags(group, nick, flags)
		webinfo.response.status = "302 Found"
		webinfo.response.headers['location'] = 'info?group=' + quote_plus(group)
		return ''

	def remove_flags(self, group, nick):
		try:
			conn = get_xmlrpc_connection()
		except:
			webinfo.response.status = "302 Found"
		...
		conn.chanserv.set_access_flags(group, nick, '-*fF')
		webinfo.response.status = "302 Found"
		webinfo.response.headers['location'] = 'info?group=' + quote_plus(group)
		return ''
	...
	def settings(self, group):
		...
		return Template('groupeditsettings').render(webinfo=webinfo, conn=conn, group=group)

	def settings_commit(self, group, TOPICLOCK='OFF', SECURE='OFF', FANTASY='OFF', RESTRICTED='OFF', VERBOSE_OPS='OFF', PRIVATE='OFF', KEEPTOPIC='OFF', GUARD='OFF', LIMITFLAGS='OFF', HOLD='OFF', VERBOSE='OFF'):
		try:
			conn = get_xmlrpc_connection()
		except:
			webinfo.response.status = "302 Found"
		...
		for key in params.keys():
			try:
				conn.chanserv.set_group_flag(group, key, params[key])
			except:
				pass

		webinfo.response.status = "302 Found"
		webinfo.response.headers['location'] = 'settings?group=%s' % quote_plus(group)
		return ''
